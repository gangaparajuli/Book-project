...
if(isset($_POST['add_book'])){
 ...
 $select_book_name = mysqli_query($conn, "SELECT name FROM `books` WHERE name ='$name'") or die('query fail');

 if(mysqli_num_rows($select_book_name) >0 ){
    $message[] = 'book name already added';
 }
 else{
    $add_book_query = mysqli_query($conn, "INSERT INTO `books`(name, price, image) VALUES ('$name','$price', '$image')") or die('query fail');
